...
# get price info
def get_price(price_path):
    price = pd.read_excel(price_path, sheet_name='arabica_ratings_raw')
    columns = price.loc[0]
...
def train(data, price):
    ...
    random_accuracy = evaluate(rf_random.best_estimator_, test, test_labels['Price '])
...
if __name__ == "__main__":
    price_path = 'data.xlsx'
    price = get_price(price_path)

    data_path = 'arabica_ratings_raw.csv'
    data = get_data(data_path)

    best_random, random_accuracy = train(data=data, price=price)
